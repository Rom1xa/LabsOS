...
int parse_symbolic_mode(const char *mode_str, mode_t current_mode) {
    ...
    while (*p && *p != '\0') {
        switch (*p) {
            case 'r': 
                if (who & S_IRWXU) what |= S_IRUSR;
                if (who & S_IRWXG) what |= S_IRGRP;
                if (who & S_IRWXO) what |= S_IROTH;
                break;
            case 'w': 
                if (who & S_IRWXU) what |= S_IWUSR;
                if (who & S_IRWXG) what |= S_IWGRP;
                if (who & S_IRWXO) what |= S_IWOTH;
                break;
            case 'x': 
                if (who & S_IRWXU) what |= S_IXUSR;
                if (who & S_IRWXG) what |= S_IXGRP;
                if (who & S_IRWXO) what |= S_IXOTH;
                break;
            default:
                return -1;
        }
    ...
    switch (op) {
        ...
        case '-':
            ...
            break;
        case '=':
            if (who & S_IRWXU) new_mode &= ~S_IRWXU;
            if (who & S_IRWXG) new_mode &= ~S_IRWXG;
            if (who & S_IRWXO) new_mode &= ~S_IRWXO;
            new_mode |= what;
            break;
        default:
            return -1;
...
int main(int argc, char *argv[]) {
    ...
    if (mode_str[0] >= '0' && mode_str[0] <= '7') {
        ...
        for (int i = 2; i < argc; i++) {
            mode_t current_mode = get_file_mode(argv[i]);
            if (current_mode == 0) {
